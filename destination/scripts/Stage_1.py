...
Table1 = destination([2.45,1.45,-pi/4],'The First Table')
Table2 = destination([2.80,2.35,0],'The Second Table')
Tables = [Table1, Table2]
RedStorage = destination([-0.70,1.3,-3*pi/4],'The Red Storage',1)
...
def pick():
    ...
    gripper_poses = [Pose(Point(trans[0], trans[1], trans[2]+0.1),Quaternion(q[0],q[1],q[2],q[3])),Pose(Point(trans[0]-0.004, trans[1], trans[2]+0.01),Quaternion(q[0],q[1],q[2],q[3]))]    
...
def place():
    ...
    joints = ["torso_lift_joint", "shoulder_pan_joint", "shoulder_lift_joint", "upperarm_roll_joint",
                "elbow_flex_joint", "forearm_roll_joint", "wrist_flex_joint", "wrist_roll_joint"]
    pose =  [0.05, 1.05, 1.40, -1.77, 1.52, 0.0, 0.0, 0.0]#[[0.05, 1.32, 1.40, -0.20, 1.72, 0.0, 0.0, 0.0],
    ...
    pose =  [0.05, 1.32, 1.40, -0.2, 1.72, 0.0, 1.66, 0.0]#[[0.05, 1.32, 1.40, -0.20, 1.72, 0.0, 0.0, 0.0],
    move_group.moveToJointPosition(joints, pose, wait=True, max_velocity_scaling_factor=velocity_scale)
    rospy.sleep(0.5)
    #head_tilt(1,0,1)
...
def clean():
    ...
    for table in Tables:
        ...
        while not table.empty:
            ...
            if response1 == True:
                ...
                if colour == 'red':
                    ...
                    ColourStorage = RedStorage.objects
                elif colour == 'blue':
                    response2 = navigate(BlueStorage)
                    ColourStorage = BlueStorage.objects
                elif colour == 'yellow':
                    response2 = navigate(YellowStorage)
                    ColourStorage = YellowStorage.objects
                elif colour == 'green':     
                    response2 = navigate(GreenStorage)
                    ColourStorage = GreenStorage.objects
                else:
            ...
            if not ColourStorage:
                sc.say('All %s objects are complete' %colour,'voice_us1_mbrola')                          
            if (not RedStorage.objects) and (not YellowStorage.objects) and (not YellowStorage.objects) and (not GreenStorage.objects) :
                break
        ...
        if (not RedStorage.objects) and (not BlueStorage.objects) and (not YellowStorage.objects) and (not GreenStorage.objects):
            sc.say("All objects are put away",'voice_us1_mbrola')
            break
...
if __name__ == '__main__':
    ...
    # Setting Fetch to defualt state
    #navigate(home)
    torso(0.05, state =3)
    #head_tilt(1,0,1)
    #gripper(0.1)

    #sc.say('Hello my name is Fetch' ,'voice_us1_mbrola')
